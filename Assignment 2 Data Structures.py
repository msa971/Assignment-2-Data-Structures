...
class Queue:
    ...
    def __init__(self):
        ...
        self.step_counters = {"enqueue_counter": 0}  # initializing the enqueue step counter and setting it to 0.

    def enqueue(self, patient):  # function to add patients into the queue.
        self.queue.append(patient)
        self.step_counters["enqueue_counter"] += 1  # adding a step to the step counter after appending the patient to the queue.
...
class Hospital:
    ...
    def __init__(self):
        ...
        self.step_counters = {
            "dequeue_counter": 0,
            "pop_counter": 0,
            "find_patient_counter": 0,
            "delete_patient_counter": 0
        }
    ...
    def delete_patient(self, patient_id):  # function to delete a patient.
        if patient_id in self.patients:  # if the patients ID is found delete the patient.
            ...
            self.step_counters["delete_patient_counter"] += 1  # adding a step after deleting a patient.
        else:
    ...
    def find_patient(self, patient_id):  # function to find a patient.
        if patient_id in self.patients:
            self.step_counters["find_patient_counter"] += 1  # adding a step after finding the patient.
            patient = self.patients[patient_id]
            return patient
        else:
            print("This patient was not found.")

    def print_patient_details(self, patient):  # function to print patient details.
    ...
    def print_step_counters(self):  # function to print the step counters.
        print("Step Counters:")
        for counter, value in self.step_counters.items():  # for loop so it can loop through all the step counters in the dictionary of step counters we created above.
            print(f"{counter.capitalize().replace('_', ' ')}:",
                  value)  # prints the name of the operation and the number of steps it took.
...
update_details = {'id': '1234', 'name': 'Mahra Al Ameri', 'medical_record': 'Allergies', 'current_diagnosis': 'Fever',
                  'appointment': 'Tuesday, April 2nd 2024. 10:00am.'}
...
if updated_patient:
    ...
    hospital.print_patient_details(updated_patient)
...
if dequeued_patient:
    ...
    hospital.step_counters["dequeue_counter"] += 1  # adds a step to the dequeue function.
...
if popped_prescription:
    ...
    hospital.step_counters["pop_counter"] += 1  # adds a step to the pop step counter.
...
while True:  # while loop to print patient details given the ID.
    ...
    if patient:
        hospital.print_patient_details(patient)  # It prints the patient's details.
    break  # to break out of the loop.

hospital.delete_patient(5678)
hospital.print_step_counters()  # prints the step counter dictionary.
